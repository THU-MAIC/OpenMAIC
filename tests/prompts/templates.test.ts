...
describe('director routing contract', () => {
  ...
  test('open student question section is injected when human message present (issue #511)', () => {
    // Uses original pre-conversion messages — originalRole:'user' is the discriminator
    const messages: StatelessChatRequest['messages'] = [
      {
        id: 'm1',
        role: 'assistant',
        parts: [{ type: 'text', text: 'The gate is axisymmetric.' }],
        metadata: { originalRole: 'agent', senderName: 'Mr. Chen', agentId: 'a1' },
      },
      {
        id: 'm2',
        role: 'assistant',
        parts: [{ type: 'text', text: 'I agree!' }],
        metadata: { originalRole: 'agent', senderName: 'Xiao Ming', agentId: 'student-1' },
      },
      {
        id: 'm3',
        role: 'user',
        parts: [
          { type: 'text', text: 'But the gate is 3D — can 3D objects really be axisymmetric?' },
        ],
    ...
    expect(out).toContain('Open Student Question');
    expect(out).toContain('3D objects really be axisymmetric');
  });

  test('open student question section is absent when no human message present', () => {
    // Only agent messages — no originalRole:'user'
    const messages: StatelessChatRequest['messages'] = [
      {
        id: 'm1',
        role: 'assistant',
      ...
      },
      {
        id: 'm2',
        role: 'assistant',
        parts: [{ type: 'text', text: 'Me too!' }],
        metadata: { originalRole: 'agent', senderName: 'Li Hua', agentId: 'student-2' },
      },
    ];
    const out = buildDirectorPrompt(
      [baseAgent],
    ...
    expect(out).not.toContain('Open Student Question');
  });

  test('open student question section is absent when originalMessages not provided', () => {
    const out = buildDirectorPrompt([baseAgent], 'No history', [], 0);
    expect(out).not.toContain('Open Student Question');
  });
});
