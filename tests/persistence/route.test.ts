...
describe('embedded persistence route', () => {
  beforeEach(() => {
    ...
    vi.doMock('@/lib/persistence/user-accounts', () => ({
      ensureUserAuthSchema: vi.fn().mockResolvedValue(undefined),
    }));
    vi.doMock('@/lib/persistence/user-interactions', () => ({
      ensureUserInteractionsSchema: vi.fn().mockResolvedValue(undefined),
    }));
  });
