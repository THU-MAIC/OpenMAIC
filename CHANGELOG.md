...
- Owner identity: claiming anonymous work on sign-in. With `trustedProxyHeader`, a gateway request that also carries a valid anonymous cookie resolves with a `pendingClaim`; `POST /api/identity/claim` (same-origin JSON only; `OWNER_CLAIM_TRIGGER=auto` claims on the first such request instead) moves the anonymous owner's folders, courses, materials, agent sessions, skills, runtime sessions and asset entries to the account in one transaction, records it in the new `owner_merges` table, and drops the anonymous cookie. Same-named folders merge, a folder id the account already uses is renumbered, a colliding skill handle gets a numeric suffix, and quotas are not applied to what moves. Claims are idempotent per pair, refused for an anonymous owner another account already claimed, and never chain. A retired anonymous id writes nothing afterwards (`403 OWNER_RETIRED`), while agent runs and media generation that started before the claim follow it to the account. Every owner write takes a per-owner advisory lock first, so a write racing a claim is either moved or refused, never orphaned or deadlocked. Hosts add their own tables with `registerClaimParticipant`, and an `OwnerAuthenticator` may implement `describeStoredOwner`, `canonicalize` and `clearPendingClaim`; `principalFromStoredOwner(ownerId)` describes a stored id for work without a request. The runtime contract's `POST /runtime/learners/merge` now performs the same claim, allowed only from the request's own pending claim.
- `@openmaic/storage` 0.35.0: `reassignDocumentFolders(tx, { fromOwnerId, toOwnerId, documentOwnership })` moves one owner's folders and filing to another (merging same-named folders, renumbering colliding ids); `PgAssetStore.reassignPrincipal(fromKey, toKey)` moves a principal's entries under both principals' write locks; `PgUserSkillStore.mergeOwner(from, to)` moves skills, renaming colliding handles; and `PgUserSkillStore` takes a `resolveFinalOwner` hook that runs as the create transaction's first statement.
- Owner identity: a `trustedProxyHeader` built-in authenticator, selected with `OWNER_AUTHENTICATOR=trusted-proxy`, gives each person their own library through an identity gateway (oauth2-proxy, Authelia, a Keycloak-based proxy, an institutional reverse proxy). The gateway forwards the signed-in user (`TRUSTED_PROXY_USER_HEADER`, default `x-forwarded-user`) and a shared secret (`TRUSTED_PROXY_SECRET`, required, in `TRUSTED_PROXY_SECRET_HEADER`, default `x-openmaic-proxy-secret`); the owner is `proxy:<user>`, may publish, and gains `admin` through the optional `TRUSTED_PROXY_GROUPS_HEADER` / `TRUSTED_PROXY_ADMIN_GROUPS`. A missing or wrong secret, a missing, ambiguous or unstorable user is a `401`, never an anonymous owner. Misconfiguration, including a header name that HTTP, Next.js or a forwarding proxy sets itself, fails startup; `TRUSTED_PROXY_ADMIN_GROUPS` logs a boot warning because the groups header is trusted on the strength of the secret alone. The unset-`ACCESS_CODE` boot warning is skipped in this mode.
- Server persistence: host extension hooks, registered once from `instrumentation.ts` next to the owner authenticator and sealed on first use. `configurePersistenceHooks` adds `authorizeCreate` / `onCreate` (run once per created course inside its transaction; a refusal answers `403 CREATE_REFUSED` and a throw rolls the create back), a `library` provider for what `GET /api/stages` lists (never an id the read path would refuse), and `beforeAssetAllocate` (refuse an upload, `create` or `replace`, with any `Response` before bytes are stored or quota counted). Hooks receive an `actor` whose `source` is `'request'` (with the resolved principal) or `'background'` (an agent run; a refusal reaches the agent as a fixed message). `configureAssetByteStore` replaces the `ASSET_S3_BUCKET` switch for the request path and the asset collector alike; under `ASSET_BYTE_EGRESS=redirect` a store that does not declare `signsReadUrls` stops the server at boot. Nothing changes when no hook is registered.
...
- Owner identity: `POST /api/chat/pi` answers `401` when the configured owner authenticator rejects the request's credential, like every other owner-resolving route, instead of continuing without an owner. Under the default anonymous authenticator nothing changes.
