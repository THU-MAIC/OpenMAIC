 ...
 *
 * When OPENMAIC_API_KEY is set, requests must include:
 *   Authorization: Bearer <key>
 *
 * When OPENMAIC_API_KEY is not set, all requests are allowed (dev mode).
 */
...
export function validateApiKey(req: NextRequest): boolean {
  const expected = process.env.OPENMAIC_API_KEY;
