 ...
 *
 * `kind` and the roles an id always carries come from the configured
 * authenticator's `describeStoredOwner` (the built-ins recognize the ids they
 * mint); roles a request is granted (gateway groups) are not known here. An id
 * the authenticator does not recognize is described as `kind: 'user'` with no
...
export function principalFromStoredOwner(ownerId: string): OwnerPrincipal {
  ...
  const description = getOwnerAuthenticator().describeStoredOwner?.(ownerId);
