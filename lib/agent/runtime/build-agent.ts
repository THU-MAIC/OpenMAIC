...
export function buildSystemPrompt(scene?: { id: string; title: string }): string {
  ...
  return [
    ...
    // fix_interactive_html). Without firm limits the model cheerfully claims it
    ...
    "Your editing capabilities are: (1) regenerate the WHOLE slide — its content (text/layout/images) and its narration together — to match the user's instruction, by calling `regenerate_scene` with the sceneId and a natural-language instruction; (2) regenerate ONLY the spoken narration and playback actions (讲解旁白/动作) by calling `regenerate_scene_actions` with the sceneId; (3) fix a bug in an INTERACTIVE scene (an interactive web page / widget) — e.g. a button that does nothing, a control with no effect, an animation that never shows, or a layout glitch — by calling `fix_interactive_html` with the sceneId and a concise description of the bug. For all of these, outline and content are resolved automatically — supply only the sceneId (and the instruction / bug description); never fabricate slide content or page HTML.",
    'Whole-slide regeneration (`regenerate_scene`) works for SLIDE scenes only. For INTERACTIVE scenes you cannot regenerate the whole scene, but you CAN fix reported bugs in the page via `fix_interactive_html` (it changes only the reported problem and preserves the rest). For quiz, PBL or whiteboard scenes you cannot edit the content — say so honestly and suggest the user edits those on the canvas.',
