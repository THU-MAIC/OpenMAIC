...
| The store refused the write as a matter of policy (`DocumentWriteRefusedError`, recognized by class or, for one from another copy of the package, by name and shape: `isDocumentWriteRefusedError`) | `403` | the refusal's own code, e.g. `CREATE_REFUSED` | Throw `HttpDocumentStoreError`; nothing was written |
