...
| `HEAD` | `/assets/{id}/content` | Read the identity headers without the bytes. | `200`, no body |
...
The headers and the body of a byte response MUST be produced from **one transactional snapshot**. Reading the revision and media type in one query and streaming the bytes in another lets a concurrent `replace` pair revision *n* headers with revision *n+1* bytes, which the client then caches as authoritative. The browser backend closes the same window deliberately, reading the registry entry in the same transaction as the bytes.
...
The revalidation mechanism reads two non-safelisted response headers, so a cross-origin server MUST send `Access-Control-Expose-Headers: X-Asset-Revision, X-Error-Code` on `GET` and `HEAD`, error responses included. Without the first, the revision reads as absent and the client's cache either never validates or validates as always-fresh. Without the second, a client cannot classify any `HEAD` error and falls back to a full byte `GET` on every miss — so the mechanism `HEAD` exists to provide stops working, silently, in exactly the deployment shape this section addresses.
...
- Bytes are written **before** the row that references them. A crash between the two leaves bytes nobody references, which costs storage and loses nothing; the reverse order would leave a registry entry pointing at bytes that were never stored.
- Bytes are deleted **after** the last row referencing them, and only by the offline collector.
- The reference count and the decision to collect are serialized on the **blob row** in the transactional store, which every byte layer keeps regardless of where the bytes themselves sit.
...
1. **The hardest race in the design disappears.** A request can no longer delete bytes another request is adopting, so no request path needs a lock on the byte row, and the write path does not have to reconcile "take a lock" with "never branch on whether these bytes already existed." The collector takes that lock instead — it runs rarely, offline, and contends with nobody.
...
The third channel, headers, is closed by **not reading it**, and this contract deliberately does not attempt to police header names. A server implementing it reads exactly `Content-Type` and `Content-Length`, plus whatever the deployment's `authenticate` hook consumes; it MUST NOT derive a principal, an owner, or a content hash from any other request header, and there is no header this contract assigns a meaning to. A header the implementation never reads smuggles nothing, so the requirement is a rule about the implementation rather than a filter on traffic.
