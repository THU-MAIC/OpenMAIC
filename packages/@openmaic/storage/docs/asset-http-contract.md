...
One consequence is worth stating rather than leaving to be discovered: because the byte write sits between claiming the row and inserting the entry, it happens while the registry transaction is open. For a byte layer inside that store this costs nothing; for an object store it means a row lock is held across a network upload, which is a consideration for how a deployment sizes `maxAssetBytes` and its connection pool.
