...
- **`PgDocumentStore`'s `assetReferencePrincipals`** (default unset) scopes that maintenance to the entries of the listed principals: a write references and commits only those, and an id naming any other principal's entry records nothing, like an unknown id. A host that partitions its asset registry per owner sets it to the writing owner's principal (plus any partition every owner may use), and gives `AssetCollector` the matching `assetReferencePrincipals(documentOwnerId)` so the one-time backfill applies the same scoping per document owner. Without it, any document naming an id commits and pins that entry, whoever holds it.
