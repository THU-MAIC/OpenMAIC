...
- **`PgDocumentStore`'s `assetReferencePrincipals`** (default unset) scopes that maintenance to the entries of the principals it returns: a write references and commits only those, and an id naming any other principal's entry records nothing, like an unknown id. It is a function of the store's bound owner, evaluated per write, so a store re-bound with `forOwner` scopes to the new owner. A host that partitions its asset registry per owner returns that owner's principal (plus any partition every owner may use), and gives `AssetCollector` the same function so the one-time backfill applies the same scoping per document owner — together with the same `documentOwnership` relation, which is where the collector reads each document's owner (`document_stages` records none). The collector refuses the function without that option; pass `false` when documents have no owners. Without it, any document naming an id commits and pins that entry, whoever holds it.
