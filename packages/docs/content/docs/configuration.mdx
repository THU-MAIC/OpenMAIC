...
Configure Tavily, Exa, Serply, Bocha, Brave, Baidu, SearXNG, or MiniMax:
...
SERPLY_API_KEY=
SERPLY_BASE_URL=https://api.serply.io  # optional override
...
Environment variables override matching YAML provider fields one by one. Keys must use provider IDs registered in the code, such as `openai`, `doubao-tts`, `openai-whisper`, `mineru`, `alidocmind`, `seedream`, `seedance`, `exa`, `serply`, `tavily`, or `searxng`. Unknown IDs cannot register custom LLM providers here.
