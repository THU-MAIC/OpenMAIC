...
First add the database connection to `.env.local`:
...
`NEXT_PUBLIC_PERSISTENCE` is a build-time variable. There is no separate persistence credential: every request is attributed to the owner the owner authenticator resolves (by default an anonymous per-browser cookie), and runtime sessions and media are partitioned by that owner. The default identity isolates browsers but authenticates nobody, so a deployment serving real users should register an owner authenticator backed by its own accounts. Earlier releases used `PERSISTENCE_DEV_TOKEN` / `NEXT_PUBLIC_PERSISTENCE_TOKEN`; both are removed and ignored, and runtime sessions stored under the old browser-minted learner key are no longer reachable. If `PERSISTENCE_DEV_TOKEN` was your only access gate, put the deployment behind `ACCESS_CODE` or a gateway, register an owner authenticator, or leave `NEXT_PUBLIC_PERSISTENCE` unset before upgrading.
