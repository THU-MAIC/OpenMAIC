...
export interface GenerationToolbarProps {
  language: 'zh-CN' | 'en-US' | 'uz';
  onLanguageChange: (lang: 'zh-CN' | 'en-US' | 'uz') => void;
  webSearch: boolean;
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            onClick={() => onLanguageChange(language === 'zh-CN' ? 'en-US' : language === 'en-US' ? 'uz' : 'zh-CN')}
            className={pillMuted}
          >
            <Globe className="size-3.5" />
            <span>{language === 'zh-CN' ? '中文' : language === 'en-US' ? 'EN' : 'UZ'}</span>
          </button>
        </TooltipTrigger>
