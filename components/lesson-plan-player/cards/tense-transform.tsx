...
import type { TenseTransformCard } from '@/lib/types/stage';

export function TenseTransformCard({ card }: { card: TenseTransformCard }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-violet-50 dark:bg-violet-900/20 rounded-xl p-4">
        <p className="text-sm text-violet-700 dark:text-violet-400">{card.instruction}</p>
        {card.hint && <p className="mt-1 text-xs text-violet-600 dark:text-violet-500 italic">Hint: {card.hint}</p>}
      </div>
      {!revealed ? (
        <button
